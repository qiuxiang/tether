...
`exec` 不再有 `timeout` 参数,也无内部 deadline。退出条件只有"进程退出"和"MCP ctx 取消"两种。
...
- `exec` 工具:
  ...
  - 实现改为:`Start` → 等 reply → `Attach` → 消费 `ProcessOutput` 累积到 `output` 字节缓冲,遇 `ProcessExit` 正常返回,遇 `ctx.Done()` 发 `Detach` 并返回成功结果(带 `timed_out: true`)。
- `start_process` 工具:
  - 删除 `name` 参数。
  - 新增 `description` 参数(可选)。
