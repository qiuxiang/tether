...
| `type` | 字段 | 说明 |
|---|---|---|
| `exec` | `msg_id, cmd, cwd?, env?, stdin?, timeout?` | 一次性命令，流式回输出（始终在 PTY 内运行） |
| `exec_cancel` | `msg_id` | 取消进行中的 exec（reuse msg_id 做 handle） |
| `start` | `msg_id, process_id, cmd, cwd?, env?, name?` | 启动后台进程（始终在 PTY 内运行） |
| `stdin` | `process_id, data` | fire-and-forget，无 reply |
| `kill` | `msg_id, process_id, signal?` | 终止进程 |
| `capture_screen` | `msg_id, process_id, start_line?, end_line?` | 把进程 stdout/stderr 喂进虚拟终端 (vt10x, 200×10000)，返回渲染后的纯文本行 + 光标 + 总行数 |
...
**Stdin** 不返回 reply：如果进程已退出，stdin 写入静默丢弃，AI 在下次 `list`/`capture_screen` 看到状态变化。
...
**PTY 模式（始终开启）：**
- 所有进程都在 PTY 内拉起，stdin 是 raw（AI 发 `"r"` 单字符就被程序收到）
- stdout/stderr 合并；输出含 ANSI 转义
- 跨平台：unix 用 PTY，Windows 用 ConPTY（统一通过 `go-pty`）
- pager/REPL/prompt 等待输入时由调用方通过 `capture_screen` + `send_stdin` 处理
...
| `list_processes` | `device?, limit?=50, status_filter?="all"` | `[{process_id, name?, cmd, status, started_at, last_active_at, exit_code?, log_path}]` |
| `capture_screen` | `device, process_id, start_line?, end_line?` | `{lines, cursor, cols, total_lines}` |
...
- `capture_screen` 的 scrollback 上限 10000 行；超过的内容仍在 log 文件中，通过 `list_processes` 的 `log_path` + `file_transfer` 取回。
