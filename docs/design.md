...
- **exec 处理**：收到 `exec` frame 后，按结构化 argv 直接 spawn 普通子进程（不经 shell、非 PTY，argv[0] 由 PATH 解析），独立进程组；等待退出或超时（默认 30s）后 kill 整个进程组（Windows 上仅 kill 直接子进程），返回 `{stdout, stderr, exit_code, timed_out, truncated}`；stdout/stderr 各限 4 MiB，超出截断并置 `truncated=true`
...
| `type` | 字段 | 说明 |
|---|---|---|
| `exec` | `msg_id, target, cmd, cwd?, env?, timeout?` | 同步子进程（node 端按 OS 选 shell：`sh -c` / `cmd /c`）；node 等待退出或超时后回一个 `reply` |
...
- 命令字符串由 node 经本机 shell 执行（Unix `sh -c`，Windows `cmd /c`），在独立进程组内运行（非 PTY）
...
**约定：**
- `cmd` 字符串由目标 node 经本机 shell 执行（Unix `sh -c`，Windows `cmd /c`）
