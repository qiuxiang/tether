...
- **PTY 支持**：unix 用 `github.com/creack/pty`；windows 上 `tty:true` 直接返回 `not supported`
...
- Windows 上报错 `not supported`，AI 退回非 PTY 模式
...
tether agent              # 设备 agent（被管设备）
tether agent --once --tail  # 调试：前台 + 打印 inbound/outbound 帧
```
...
**Agent 配置（`~/.config/tether-agent/config.yaml`）：**

```yaml
hub_url: "wss://tether.example.com/device"
token: "xxx"
hostname_override: ""      # 默认 os.Hostname()
log_dir: "~/.local/share/tether-agent/logs"
```
...
**Service 集成：**
- Linux：systemd unit 模板放 `dist/systemd/tether.service` 和 `tether-agent.service`
...
2. **Windows ConPTY**（当前 windows 不支持 tty）
3. **设备掉线期间的 output 补传**（当前掉线就 device_offline）
