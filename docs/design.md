...
- **exec 处理**：收到 `exec` frame 后，经本机 shell（Unix `sh -c`，Windows `cmd /c`）起普通子进程（非 PTY），独立进程组；等待退出或超时（默认 30s）后 kill 整个进程组（Windows 上仅 kill 直接子进程），返回 `{stdout, stderr, exit_code, timed_out, truncated}`；stdout/stderr 各限 4 MiB，超出截断并置 `truncated=true`
